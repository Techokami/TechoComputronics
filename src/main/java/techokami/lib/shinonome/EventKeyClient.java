package techokami.lib.shinonome;
...
public class EventKeyClient {
	...
	public void asieEvent(RenderPlayerEvent.SetArmorModel event) {
		if(EventKeyServer.isEntityNano(event.entityPlayer) && event.slot == 1 && (event.stack == null || event.stack.getItem() == TechoLibMod.itemKey)) {
			// Render the key on top.
			ItemStack key = new ItemStack(TechoLibMod.itemKey, 1);
			try {
            ...
            return;
		} else if(event.stack != null && event.stack.getItem() == TechoLibMod.itemKey) {
			event.result = 0;
			return;
		}
	}
